...
| `last_reconcile_error` | `text` (nullable) | Human-readable reason for the most recent reconcile failure, written together with `last_reconcile_failed_at`. NULL when the book has never failed. **Not cleared on success** — `last_reconcile_failed_at` is the authority on whether the failure is current, and keeping the text lets an operator see what the last one was. Exists because the observer step and the operator both read the database, not the pod logs: without it "the book is failing" cannot be told apart from "the book is failing for an unknown reason". Note that the benign `NoBoc` outcome also stamps a failure, with its own fixed text, so a non-NULL value here means the reason is *recorded*, not that it is severe. |
| `reconcile_attempts` | `integer` default `0` | Diagnostic counter. |
