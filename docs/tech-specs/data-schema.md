...
| `is_subscription` | `boolean` default `false` | `true` when `InferenceOrderPlaced.flags` had `FLAG_SUBSCRIPTION` (0x40) set. Since v4.0.32 a subscription has no event of its own — it is an ordinary order carrying that bit, so the flag is the only thing distinguishing one. Rests in the book like any other bid; flagged for diagnostics. |
...
| `close_kind` | `text` (nullable) | Terminal close type: one of `STOPPED`, `DISPUTE_RESOLVED`, `RECLAIMED`, `DESTROYED`, `PROBE_BURNED`. Enforced by a CHECK constraint. `RECLAIMED` is no longer written — `StreamReclaimed` has had no emitter since v4.0.32 — but stays in the constraint for rows written before then. |
