...
| `raw_events_created_at_chain_idx` (desc) | **Chain** time (`created_at_chain`) — analytics only. Not the index behind "what arrived in the last hour": that is ingest time, see below. |
| `raw_events_created_at_idx` | **Ingest** time (`created_at`). Backs every run-window query of the e2e observer (`pending_projection_since`, `count_undecodable_since`, `inference_books_with_events_since`, `inference_anchored_books_since`) and the operator's "what arrived in the last N minutes". Full, not partial, unlike both `src_address` indices: a run window needs rows in any processing state, and at the tail of a run most of them are already processed. Building it takes SHARE on `raw_events`. `CONCURRENTLY` is declined, not unavailable: sqlx runs a migration outside a transaction when its file opens with `-- no-transaction`, but a failed concurrent build leaves an INVALID index to find and drop by hand — a worse failure mode than a slow deploy for an index this cheap. Expect a longer deploy on a large table. |
...
| `last_reconcile_error` | `text` (nullable) | Human-readable reason for the most recent reconcile failure, written together with `last_reconcile_failed_at` and **cleared together with it**. NULL means no failure is current — the book never failed, or it failed and recovered. The pair is never split: a text whose timestamp is gone is the stale artefact that makes the text untrustworthy. Two writers clear both columns — the discovery visibility stamp (`advance_sweep_and_maybe_stamp`) and a Queue B refresh pass that completes (`clear_failure`) — and `stamp_failure` sets both. Exists because the observer step and the operator both read the database, not the pod logs: without it "the book is failing" cannot be told apart from "the book is failing for an unknown reason". Note that the benign `NoBoc` outcome also stamps a failure, with its own fixed text, so a non-NULL value here means the reason is *recorded*, not that it is severe. |
| `reconcile_attempts` | `integer` default `0` | Diagnostic counter. |
...
| `is_subscription` | `boolean` default `false` | `true` when `InferenceOrderPlaced` carried the `FLAG_SUBSCRIPTION` bit (`0x40`, [`modifiers.sol`](../../contracts/airegistry/modifiers/modifiers.sol)) in its `flags` field — spec §8, a standing bid throttled by a weekly budget. There is no separate subscription event: a subscription is an ordinary placement with that bit set, and it rests in the book like any other bid. |
| `status` | `text` CHECK `IN ('OPEN','FILLED','CANCELLED','EXPIRED')` | Order lifecycle. Depth aggregation filters on `status = 'OPEN' AND amount_remaining > 0`. A SELL offer is a one-deal slot consumed on match; a BUY maker reduces across fills. `EXPIRED` has **two writers**. `InferenceOrderExpired` is the ordinary one, emitted when the book drops an order whose deadline passed. The second is `InferenceRefunded`: a continuation resumed past its deadline emits ONLY a refund and no expiry event at all, so that projector compares the stored `deadline` against the event's chain time and writes `EXPIRED` when it has passed (see [Inference projectors](indexer.md#inference-projectors) and migration 0005). Neither derives the status from wall-clock — a row whose `deadline` already lies in the past stays `OPEN` until one of those two events lands; the refund path's comparison is allowed precisely because the chain has already reported the order gone and the deadline only answers why. It overrides a provisional sweep-cancel (`swept_at` NOT NULL, which the sweep set precisely because the order had vanished from the book) but never a `FILLED` row or a real event-cancel. Once written it is **fully terminal in both directions**: a replayed `InferenceOrderPlaced` cannot reopen the row (the upsert's conflict arm excludes `EXPIRED` alongside `FILLED` and `CANCELLED`), a late `InferenceOrderCancelled` cannot demote it to `CANCELLED`, and a late `InferenceFilled` neither decrements `amount_remaining` nor flips it to `FILLED`. |
...
| `token_contract` | `text` (nullable) | Deal TokenContract. **Two writers.** (1) The `InferenceOrderPlaced` handler decodes the mandatory `tokenContract` field strictly (a missing field fails the projection rather than inserting a NULL) and normalizes a decoded zero address to NULL — on chain a BUY carries the zero address there, so `token_contract` is non-NULL only on SELL rows. A live SELL with a NULL value is a LEGACY row, not a live race: `placeSellOffer` passes the TokenContract as `msg.sender` and `InferenceOrderPlaced` carries it verbatim, so every SELL the current projector writes has it. Rows written before the projector decoded the field at all (commit 9350896) carry NULL, nothing backfilled them, and the read API refuses TokenContract lookups against such a book until the reconciler's sweep repair fills them from `getOrder`. (2) The inference reconciler's phantom-cancel sweep (`repair_order_fields`) fills it from the `tokenContract` the `getOrder` probe already returns, but only while the column is still NULL and the getter supplies a non-zero address — repairing `token_contract` and `deadline` independently, so a BUY's zero address is left NULL. See [Inference reconciler](indexer.md#inference-reconciler). The upsert's conflict arm is NULL-preserving (`coalesce(excluded.token_contract, inference_orders.token_contract)`), so a replay of `InferenceOrderPlaced` can never erase a value the reconciler recovered from chain. |
| `deadline` | `numeric(20,0)` (nullable) | Unix seconds. **Two writers.** (1) The `InferenceOrderPlaced` handler decodes the mandatory `deadline` field strictly and normalizes a decoded 0 to NULL — zero means good-till-cancel, which on this book is a bid. A resting SELL is never GTC: the contract rejects a zero-deadline offer as malformed, so a SELL row with a NULL `deadline` is a gap, not an intentional value. A subscription bid is no exception and gets no second chance: `FLAG_SUBSCRIPTION` fixes the term at one month in the contract, so the order **carries no duration of its own** (`InferenceOrderBook.sol`, `placeBuyOrder`) and a subscription placed with deadline 0 rests with 0 on chain. There is no hidden on-chain value for the reconciler probe to recover — the probe writes only non-zero getter values — so the NULL on such a row is permanent and correct, not pending. (2) The inference reconciler's phantom-cancel sweep (`repair_order_fields`) fills it from the `deadline` the `getOrder` probe already returns, but only while the column is still NULL and the getter supplies a non-zero value — independently of `token_contract`, so a GTC bid's zero deadline is left NULL. See [Inference reconciler](indexer.md#inference-reconciler). Same NULL-preserving upsert as `token_contract`, so no replay can erase a reconciler-recovered value. |
...
| `close_kind` | `text` (nullable) | Terminal close type: one of `STOPPED`, `DISPUTE_RESOLVED`, `RECLAIMED`, `DESTROYED`, `PROBE_BURNED`. Enforced by a CHECK constraint (0001, so the set is fixed). `RECLAIMED` is accepted by the constraint but **no longer reachable**: `TokenContract.StreamReclaimed` left the ABI and its projector arm was removed as dead. A guard in `decoder.rs` fails if the event returns, so the value is kept rather than migrated away. |
