...
list — each derives its expected set from the ABI or the emit sites — and the
strongest guard of the set is not in this table at all: the projectors destructure
their DTOs exhaustively, so a field that arrives and is consumed by nobody is a
