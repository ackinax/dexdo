...
| `InferenceOrderBook.InferenceOrderExpired` | Same effect as `InferenceOrderCancelled` — the order left the book unfilled, and the read model has no separate terminal state for an expiry. Shares one handler with the cancel, including the `swept_at → NULL` reset: the event is authoritative where the phantom sweep is only provisional, closing the row at the chain's own moment instead of whenever the next sweep happens to probe. |
| `InferenceOrderBook.InferenceExecuted` / `InferenceRefunded` / `InferenceOrderRejected` / `InferenceOrderCancelRejected` / `InferenceOrderBookDeployed` | Observability-only — no `inference_orders` mutation. `InferenceRefunded` gained an `orderId` in v4.0.32, but a refund is not by itself a close (an IOC leftover refunds while its filled part stands), so the closing signal remains `InferenceOrderExpired` / `InferenceOrderCancelled` plus the reconciler sweep. `InferenceOrderRejected` reports a submission refused before it ever became an order — it has no order id, and its `refund` is a note-side balance move, not book state. `InferenceOrderCancelRejected` reports a cancel that matched no resting order (`reason = 0`) or came from a foreign owner (`reason = 1`) — the book is unchanged by construction, so there is nothing to project. |
...
- **Expired** — `InferenceOrderExpired` → `CANCELLED`. Correct from events alone (since v4.0.32).
- **Placed-but-never-rested** (FOK/POST_ONLY rejected, IOC/MARKET leftover) — the only on-chain signal is a `InferenceRefunded`. It names an `orderId` since v4.0.32, but a refund does not by itself mean the order is gone, so the projector still cannot close the row on it alone. Left untreated, the `InferenceOrderPlaced` row sits `OPEN` and pollutes depth (a phantom level).
...
> **Contract-side follow-up — mostly delivered in v4.0.32.** This asked for `flags` on `OrderPlaced`, an `orderId` on `Refunded`, or an explicit close event; the contract now ships all three (`flags`, `InferenceRefunded.orderId`, `InferenceOrderExpired`). Expiry is event-exact as a result. What remains is the taker case: `flags` says an order was submitted `IOC`/`FOK`/`MARKET`, but not whether *this* placement rested, and a refund does not distinguish "leftover cancelled" from "partial fill refunded". Making depth fully event-exact — and retiring the getter sweep — needs a close event on that path too.
...
| `SellerBondFunded` / `ProbeAccepted` / `ShellWithdrawn` / `TicksClaimed` | No-op beyond skeleton seed — these carry no deal-level state the SETTLEMENT read-model needs. `TicksClaimed` reports the claim pipeline mid-flight (`trusted` = already final, `claimed` = the newest still-contestable claim); only the final half is money owed, and `TickFinalized` already carries that into `inference_ticks`. |

`StreamReclaimed` is declared in the ABI but has been emitted by nothing since v4.0.32 (the contract dropped both its emit sites and its `StreamReclaimedEmit` id), so it has no handler. `close_kind = 'RECLAIMED'` stays in the CHECK constraint for rows written before then.
