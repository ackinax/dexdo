...
| Metric | Type | What it measures | Source |
| --- | --- | --- | --- |
| `indexer_inference_orphans_dropped` | counter | Inference events (`InferenceFilled`, `InferenceOrderCancelled`, `InferenceOrderExpired`, `InferenceRefunded`) dead-lettered because their parent `InferenceOrderPlaced` never arrived within `inference_orphan_cutoff_ms` | in-process counter, polled each refresh |
