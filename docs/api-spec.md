...
| `contractVersion` | STRING \| null | Version of the deployed order-book contract backing this market (the `inference_markets.version` column, e.g. `"4.0.30"`). Distinct from `model.version`, which is the AI model's own version. `null` when the contract version is not yet known on chain. |
...
| Field | Type | Description |
| --- | --- | --- |
| `contractVersion` | STRING \| null | Version of the deployed order-book contract for this book (the `inference_markets.version` column, e.g. `"4.0.30"`). Same value as `contractVersion` in [`/api/v1/inference/markets`](#inference-markets) for the same `inferenceOrderBookAddress`. `null` when the contract version is not yet known on chain. |
