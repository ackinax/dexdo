...
contract TokenContract is AiRegistryModifiers {
    ...
    uint256 constant PRIVATE_NOTE_CODE_HASH  = 0x87bd964e2170630f7e6cc18935a5751569b630aae1b80b0057604ffe64f21a5a;
    ...
    function fundFromOrderBook(address buyerNote, uint256 buyerPubkey, uint8 dealFlags) public {
        ensureBalance();
        ...
        // match produced. `_iobHash == 0` means the RootPN reply has not arrived yet, which is
        // equally not a book call. The accept-then-refund path below is for the BOOK's own
        // non-fundable fills; anything else is simply not answered.
        if (_iobHash == 0
    ...
    ///         ones: `fundSellerBond` accepts the bond from the moment the deal exists, since `2P`
    ...
    function close(address payoutAddress) public onlyOwnerPubkey(_sellerPubkey) accept {
        ...
        // `selfdestruct` would send it to a caller-chosen address rather than to `_sellerNote`,
        // and a cross-dapp sweep does not survive the trip.
        _returnBond();
        emit ContractDestroyed{dest: address.makeAddrExtern(ContractDestroyedEmit, bitCntAddress)}(address(this));
    ...
    function getShellBalance() external pure returns (uint128) {
