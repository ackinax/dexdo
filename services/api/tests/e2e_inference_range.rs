...
//
// ## Why this file was gone, and why it is back
//
// The v4.0.33 contract sync (`4a6b0ce`, 2026-08-06) deleted it along with five
// other inference binaries. For those five the reason was the obvious one:
// `advance`, `reclaimOnTimeout`, `abandonDispute`, `placeSubscription` and
// `pokeSubscription` left the ABI, and a test whose subject is a removed call
// has nothing to say.
//
// This one was collateral damage. Every contract call its SUBJECT rests on
// survived the sync untouched — `addRangeEvent`, `resolveRange` and
// `onWeeklyMedian` are still in `OracleEventList`, `requestWeeklyMedian` and
// `reportFinalized` still in `InferenceOrderBook`. What broke was its SETUP:
// to have a price at all it must run a deal to a close, and closing went
// through `advance`.
//
// So the restoration is a substitution in one section and nothing else:
//
//   before v4.0.33            now
//   postSellerBond            fundDeal
//   advance                   acceptProbe
//
// `acceptProbe` sets `_ticksFinalized = 1` (`TokenContract.sol:1063`), and the
// close routes through `_settleFees` → `_reportFinalized` (`:583`), which
// publishes to the book whenever `_ticksFinalized != 0` (`:597`).
// `MIN_LIQUIDITY` is 1 (`InferenceOrderBook.sol:248`), so that single probe
// tick is the whole liquidity the median needs — exactly as the one `advance`
...
//
// ## Two notes, where the deleted version had one
//
// The deal is denominated in SHELL and the prediction market in NACKL, and a
// note's currency is fixed by its constructor, so no single note can do both.
// The old file did it with one because `PN-INF` did not exist yet — the group
// was created by wave 5 of the indexer matrix, and `e2e_inference_range_link`
// already splits the same way for the same reason.
//
...
/// The pair of bond messages that must both land before `open` — see the wait
/// below for why neither is synchronous.
const BOND_TICKS: u32 = 30;

const SHELL: u128 = 1_000_000_000;

/// The note that owns the book, the `TokenContract` and both sides of the deal.
/// `PN-INF`, because the escrow a deal locks comes out of the note's SHELL
/// balance and only the constructor writes it. 23 was the group's spare index;
/// see `pn_pool_split.rs` for what taking the last one costs.
const INFERENCE_NOTE_INDEX: usize = 23;
/// The note that deploys the prediction market. `PN-API`, because the market is
/// NACKL.
const API_NOTE_INDEX: usize = 6;
...
/// probe tick is ever finalized here, and one is all the median needs.
...
/// (`:906`). The note sends with `bounce:true`, so a short bond comes back and
/// leaves no trace but `bond_funded: false` two steps later — which is how it
/// cost dexdo pipeline #293 a whole run.
const SELLER_BOND: u128 = 2 * RANGE_PRICE;
...
/// `FLAG_IOC` (`InferenceOrderBook.sol:129`): cross what rests and leave no
/// remainder. A resting remainder would be a second order this scene never
/// accounts for.
...
/// `PROBE_WINDOW` (`contracts/airegistry/modifiers/modifiers.sol:25`) is 180s,
/// and `TokenContract.sol:1034` compares it against the CHAIN's clock rather
/// than this host's. The extra 10s covers that difference: accepting one second
/// early reverts with `ERR_SETTLE_WINDOW_OPEN`, which reads as "the seller could
/// not accept" rather than "the test did not wait".
///
/// This wait is incompressible — it is not a `sleep` standing in for a fact, it
/// IS the fact the contract measures, and there is no earlier observable that
/// says the window has passed.
...
async fn a_range_market_resolves_into_the_bucket_its_books_price_falls_in() {
    ...
    let note = inf_pool.notes[INFERENCE_NOTE_INDEX % inf_pool.notes.len()].clone();
    ...
    eprintln!(
        "[e2e_range] deal_note={} market_note={} model_name={model_name}",
        note.address, market_note.address
    );

    // ── 1. a deal, run to a close, so the book has a price at all ─────────
    //
    // Self-trade: the parties are not the subject here, the price is (telling
    // the two sides apart is `e2e_inference_stream`'s job). What matters is
    // that the deal is genuinely closed — a match alone publishes nothing, by
    // design, so that orders nobody honours cannot move the reference price.
    ...
    // AND `_buyerBondFunded`; the seller's is set by the `fundDeal` just sent
    // and the buyer's by the `fundBuyerBond` its own note emitted back on the
    // fill (`:721-727` says in as many words that it "arrives later"). Sending
    // `open` straight after `fundDeal` races both and reverts in the compute
    // phase with code 621 — which reads as "open is broken" rather than "open
    // was early". The seller side has a flag; the buyer side does not, which is
    // why `bond_held > 0` stands in for one.
    ...
    // Through `finish` rather than `expect`: a revert here strands the note in
    // a funded deal, and a panic would skip the cancel-all that releases its
    // resting orders.
    if let Err(err) = dex
        .token_contract_open(
            &tc,
    ...
    dex.token_contract_accept_probe(&tc, signer_of(&note)).await.expect("acceptProbe accepted");
    ...
    //
    // From the NACKL note: the market's collateral is NACKL and the deal above
    // was SHELL, and one note cannot hold both.
    dex.deploy_pmp(
        &market_note.address,
        ParamsOfDeployPmp {
